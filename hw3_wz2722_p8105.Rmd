...
# 1.Check structure of the dataset
str(ny_noaa)
# 2.Convert tmax and tmin to numeric 
ny_noaa <- ny_noaa %>%
  mutate(
...
ny_noaa <- ny_noaa %>%
  mutate(
    ...
    month = month(date),
    day = day(date),
    tmax = tmax / 10,      # Convert temperature from tenths of degrees C
    tmin = tmin / 10,
    prcp = prcp / 10,      # Convert precipitation from tenths of mm to mm
    snow = snow            # Snowfall already in mm
  )

# Check most common snowfall values
...
```
```{r}
#Part 2.Create a double-sided panel chart displaying the average highest temperatures for January and July at each site over the years
# 1.creaate a double-sided panel
library(ggplot2)

# January and July data
january_data <- ny_noaa %>%
  filter(month == 1, !is.na(tmax)) %>%
  group_by(year, id) %>%
  summarize(mean_tmax = mean(tmax, na.rm = TRUE))

july_data <- ny_noaa %>%
  filter(month == 7, !is.na(tmax)) %>%
  group_by(year, id) %>%
  summarize(mean_tmax = mean(tmax, na.rm = TRUE))

# Plotting
ggplot(january_data, aes(x = year, y = mean_tmax, color = id)) +
  geom_line() +
  ggtitle("Average Max Temperature in January by Station")

ggplot(july_data, aes(x = year, y = mean_tmax, color = id)) +
  geom_line() +
  ggtitle("Average Max Temperature in July by Station")
```


```{r}
#Part 3.Tmax, Tmin, and Snow Distribution
# tmax vs tmin
...
# Snowfall distribution for values > 0 and < 100
snowfall_data <- ny_noaa %>%
  filter(snow > 0, snow < 100)

ggplot(snowfall_data, aes(x = snow)) +
  geom_histogram(binwidth = 5, fill = "blue", color = "black") +
  labs(title = "Snowfall Distribution (0 < Snowfall < 100)", x = "Snowfall (mm)", y = "Frequency")
```
#problem 2.
